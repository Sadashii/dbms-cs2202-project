      ...
      >
        ...
        >
          <div className="flex justify-between items-start">
            <span className="uppercase tracking-widest font-bold text-xs opacity-90 drop-shadow-sm">{card.type}</span>
            <div className="flex items-center gap-2 bg-black/20 px-2.5 py-1 rounded-full backdrop-blur-sm">
               <span className={`w-2 h-2 rounded-full shadow-[0_0_8px_currentColor] ${getStatusColor(card.status)}`}></span>
               <span className="text-[10px] uppercase tracking-wider font-semibold opacity-100">{card.status}</span>
            </div>
          </div>
          
          <div className="font-mono text-2xl tracking-[0.15em] mt-2 drop-shadow-md">{card.maskedNumber}</div>
          
          <div className="flex justify-between items-end">
            <div>
              <p className="text-[9px] opacity-70 uppercase tracking-widest mb-1">Card Holder</p>
              <p className="font-medium text-sm tracking-widest uppercase drop-shadow-sm">VaultPay User</p>
            </div>
            <div className="text-right">
              <p className="text-[9px] opacity-70 uppercase tracking-widest mb-1">Expires</p>
              <p className="font-mono text-sm drop-shadow-sm">{formatExpiry(card.expiryDate)}</p>
            </div>
          </div>
        </div>
          ...
          <div className="px-6 flex-1 flex flex-col justify-center">
            <div className="flex items-center gap-2 mt-2">
              ...
              <div className="flex-1 h-8 bg-gray-200 rounded text-black flex items-center px-3 overflow-hidden relative">
                ...
                <span className="text-gray-500 opacity-60 text-[10px] italic z-10">Authorized Signature</span>
              </div>
              {/* CVV Box */}
