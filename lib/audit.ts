...
/**
 * createAuditLog
 * Unified helper to log application events to the AuditLog collection.
 * Captures IP and User Agent automatically from request headers.
 */
export async function createAuditLog(options: AuditLogOptions) {
    try {
        ...
        // Ensure payloads are stored as stringified JSON if they are objects
        const formattedPayload = {
            previousState: options.payload?.previousState ? JSON.stringify(options.payload.previousState) : undefined,
            newState: options.payload?.newState ? JSON.stringify(options.payload.newState) : undefined,
        ...
        // We log the error but don't throw, as audit logging shouldn't crash the main business logic
