...
import { backfillMissingCustomerIds, normalizeCustomerId } from "@/lib/customerId";
...
export async function POST(req: Request) {
  try {
    ...
    const isOldPasswordValid = await bcrypt.compare(oldPassword, latestPassword.hash);
    ...
    const isSamePassword = await bcrypt.compare(newPassword, latestPassword.hash);
    if (isSamePassword) {
      return NextResponse.json(
