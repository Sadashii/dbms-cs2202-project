...
const generateRef = () => `SCH-${Math.floor(Math.random() * 90000) + 10000}-${Date.now().toString().slice(-4)}`;
...
export async function POST(request: Request) {
    try {
        ...
        const { fromAccountId, toAccountNumber, amount, memo, frequency, startDate } = body;

        // Verify Source Account
        const sourceAccount = await Account.findOne({ _id: fromAccountId, userId: user.userId, currentStatus: "Active" });
        if (!sourceAccount) return NextResponse.json({ message: "Invalid or inactive source account." }, { status: 400 });

        // Verify Destination Account (For demo purposes, we enforce internal transfers to known accounts)
        const targetAccount = await Account.findOne({ accountNumber: toAccountNumber });
        if (!targetAccount) return NextResponse.json({ message: "Recipient account not found." }, { status: 404 });
        ...
        return NextResponse.json({ message: "Transfer scheduled successfully", schedule: newSchedule }, { status: 201 });
    } catch (error: any) {
        return NextResponse.json({ message: error.message }, { status: 500 });
    }
}
...
export async function PATCH(request: Request) {
    try {
        ...
        return NextResponse.json({ message: `Schedule ${currentStatus} successfully.`, schedule }, { status: 200 });
    } catch (error: any) {
        return NextResponse.json({ message: error.message }, { status: 500 });
    }
}
