...
import { pickLatestDocumentsByType, resolveKycReviewBundle } from "@/lib/kycWorkflow";
  ...
  try {
    ...
    if (relatedAccountRequest) {
      ...
      const latestDocs = pickLatestDocumentsByType(matchingDocs).filter(
        (doc) => doc.documentType === "PAN" || doc.documentType === "Aadhar",
      );

      const hasRejectedDoc = latestDocs.some((doc) => doc.currentStatus === "Rejected");
      const allCoreDocsVerified =
        latestDocs.length === 2 && latestDocs.every((doc) => doc.currentStatus === "Verified");
      ...
      } else if (!allCoreDocsVerified && relatedAccountRequest.currentStatus !== "Approved") {
