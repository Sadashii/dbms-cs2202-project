...
export async function GET(req: Request) {
    try {
        ...
        const openTickets = await Complaint.countDocuments({
            currentStatus: { $in: ["Open", "In-Progress", "Re-Opened"] },
        });
        const resolvedTickets = await Complaint.countDocuments({
            currentStatus: "Resolved",
        });
        const closedTickets = await Complaint.countDocuments({
        ...
        if (resolvedData.length > 0) {
            avgResolutionHours =
                resolvedData[0].avgResolutionTimeMs / (1000 * 60 * 60);
        }
