...
export async function PATCH(req: Request) {
    ...
    try {
        ...
        if (dbStatus === "Approved") {
            const account = await Account.findById(loan.accountId).session(
                session,
            );
            if (!account)
                throw new Error("Linked disbursement account not found.");
        ...
        await createAuditLog({
            ...
            actionType:
                dbStatus === "Approved" ? "LOAN_APPROVED" : "LOAN_REJECTED",
            category: "Financial",
        ...
        return NextResponse.json({
            success: true,
            message: `Loan ${dbStatus} successfully.`,
        });
    } catch (error: any) {
        await session.abortTransaction();
        console.error("Loan Approval Error:", error.message);
        return NextResponse.json(
            { message: error.message || "Failed to process loan." },
            { status: 400 },
        );
    } finally {
        session.endSession();
