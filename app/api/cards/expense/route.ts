...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!cardId || !amount || isNaN(amount) || amount <= 0) {
            return NextResponse.json(
                { message: "Invalid card ID or amount." },
                { status: 400 },
            );
        }
        ...
        if (card.currentStatus !== "Active") {
            return NextResponse.json(
                {
                    message: `Card is ${card.currentStatus.toLowerCase()} and cannot be used.`,
                },
                { status: 400 },
            );
        }

        if (isOnline && !card.isOnlineEnabled)
            return NextResponse.json(
                { message: "Online transactions are disabled." },
                { status: 400 },
            );
        if (isInternational && !card.isInternationalEnabled)
            return NextResponse.json(
                { message: "International usage is disabled." },
                { status: 400 },
            );

        if (
            isContactless &&
            amount > parseFloat(card.limits.contactlessLimit.toString())
        ) {
            return NextResponse.json(
                {
                    message: `Amount exceeds contactless limit of ₹${parseFloat(card.limits.contactlessLimit.toString()).toLocaleString()}.`,
                },
                { status: 400 },
            );
        }
        ...
        if (isOnline || isATM) {
            ...
            const todayTxns = await Transaction.find({
                ...
                "metadata.isOnline": isOnline ? true : { $exists: true },
            ...
            const spentToday = ledgerEntries.reduce(
                (sum, entry) =>
                    sum + Math.abs(parseFloat(entry.amount?.toString() || "0")),
                0,
            );
            const limit = isOnline
                ? parseFloat(card.limits.dailyOnlineLimit.toString())
                : parseFloat(card.limits.dailyWithdrawalLimit.toString());
            const limitName = isOnline ? "Online" : "ATM";

            if (spentToday + amount > limit) {
                return NextResponse.json(
                    {
                        message: `Daily ${limitName} limit reached. Remaining: ₹${(limit - spentToday).toLocaleString()}`,
                    },
                    { status: 400 },
                );
            }
            ...
            [
                {
                    ...
                    metadata: {
                        ...
                        cardId: card._id,
                        merchant:
                            merchant || (isATM ? "ATM Machine" : "Merchant"),
                        isOnline,
                        isInternational,
                        isContactless,
                        isATM,
        ...
        if (card.cardType === "Credit") {
            ...
            card.limits.outstandingAmount =
                mongoose.Types.Decimal128.fromString(
                    (currentOutstanding + transactionAmount).toFixed(2),
                );
            ...
            const account = await Account.findById(card.accountId).session(
                session,
            );
            if (!account) throw new Error("Linked account not found.");
            if (parseFloat(account.balance.toString()) < transactionAmount)
        ...
        return NextResponse.json(
            { message: "Expense simulated successfully.", referenceId },
            { status: 200 },
        );
    } catch (error: any) {
