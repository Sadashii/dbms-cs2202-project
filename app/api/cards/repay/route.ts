...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!cardId || !accountId || !amount || isNaN(amount) || amount <= 0) {
            return NextResponse.json(
                { message: "Invalid card ID, account ID, or amount." },
                { status: 400 },
            );
        }
        ...
        if (card.cardType !== "Credit") {
            return NextResponse.json(
                { message: "Repayment only allowed for credit cards." },
                { status: 400 },
            );
        }
        ...
        if (
            !sourceAccount ||
            sourceAccount.userId.toString() !== decoded.userId
        ) {
            return NextResponse.json(
        ...
        const currentSourceBalance = parseFloat(
        ...
        card.limits.outstandingAmount = mongoose.Types.Decimal128.fromString(
            (currentOutstanding - repaymentAmount).toFixed(2),
        );
        ...
        return NextResponse.json(
            { message: "Repayment completed successfully.", referenceId },
            { status: 200 },
        );
    } catch (error: any) {
