...
export default function ResetPasswordPage() {
    ...
    const { getResetPasswordOTP, confirmPasswordReset, verifyConstraints } =
        useAuthContext();
    ...
    const onOTPPaste =
        (inputIndex: number) => (e: React.ClipboardEvent<HTMLInputElement>) => {
            e.preventDefault();
            ...
            setOtp((state) => {
                ...
                setTimeout(() => {
                    document
                        .getElementById(
                            `reset-otp-${Math.min(inputIndex + pastedData.length, newOtp.length - 1)}`,
                        )
                        ?.focus();
                }, 0);
    ...
    const onOTPEnter =
        (inputIndex: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (char && inputIndex < 5) {
                setTimeout(() => {
                    document
                        .getElementById(`reset-otp-${inputIndex + 1}`)
                        ?.focus();
                }, 0);
            }
        };
    ...
    const onOTPKeyDown =
        (inputIndex: number) => (e: React.KeyboardEvent<HTMLInputElement>) => {
                ...
                >
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                        <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">
                            Use this OTP to finish updating your password
                            securely.
                        </p>
                        <button
                            type="button"
    ...
    const handleRequestOTP = async (e: React.FormEvent) => {
        ...
        const result = await getResetPasswordOTP(
            customerId,
            email,
            oldPassword,
            newPassword,
        ...
        <div className="flex min-h-screen flex-1 flex-col justify-center bg-white px-6 py-12 transition-colors duration-300 dark:bg-slate-950 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-lg">
                ...
                <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
                    Verify your identity with customer ID, email, password, and
                    a secure OTP.
                </p>
            </div>
            ...
            <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-lg rounded-2xl border border-gray-200 bg-white p-8 shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900">
                ...
                <p className="mt-8 text-center text-sm text-gray-500 dark:text-gray-500">
                    ...
                    <Link
                        href="/auth/login"
                        className="font-semibold leading-6 text-blue-600 transition-colors hover:text-blue-500 dark:text-blue-400"
                    >
                        Back to sign in
                    </Link>
                </p>
