...
export default function AdminWithdrawPage() {
    ...
    const handleVerifyAccount = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.account.currentStatus !== 'Active') {
                toast.error("Account restricted from debit operations.", { id: loadingToast });
                return;
            }
    ...
    const handleWithdraw = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success(`Debit authorized! New Balance: ${data.newBalance}`, { id: loadingToast });
            setAccountNumber("");
        ...
        <div className="min-h-screen bg-white dark:bg-slate-950 p-4 md:p-8 transition-colors duration-500">
            ...
            <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter uppercase">Withdrawal Protocol</h1>
                        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 font-medium">
                            Execute direct debit operations and balance adjustments.
                        </p>
                    </div>
                    {step === 2 && (
                        <button 
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-black text-amber-900 dark:text-amber-400 uppercase tracking-tight">
                                    {step === 1 ? "Step 01: Identification" : "Step 02: Authorization"}
                                </h2>
                            ...
                            <div className="pt-6 border-t border-gray-100 dark:border-slate-800">
                                <button
                                    ...
                                    className={`px-10 py-4 rounded-2xl font-black uppercase text-xs tracking-widest text-white transition-all w-full md:w-auto shadow-lg shadow-amber-500/20 ${
                                        isLoading ? 'bg-amber-400 cursor-not-allowed opacity-50' : 'bg-amber-600 hover:bg-amber-700 active:scale-95'
                                    }`}
                                >
                                    {isLoading ? "Fetching..." : "Verify Registry"}
                                </button>
                            </div>
                        </form>
                    ) : (
                        <form onSubmit={handleWithdraw} className="p-8 md:p-12 space-y-8 animate-fade-in">
                            <div className="bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-[1.5rem] p-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-600 uppercase">Account Holder</p>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm font-black text-gray-900 dark:text-white font-mono">{accountDetails.accountNumber}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-600 uppercase">Class</p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    ...
                                    {parseFloat(amount) > accountDetails.balance && (
                                        <p className="text-[10px] font-black text-red-600 uppercase tracking-tighter animate-pulse">Insufficient Liquidity in target Node</p>
                                    )}
                                </div>

                                <div className="space-y-3">
                                    <label className="text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">Forensic Memo</label>
                                    <input
                                        type="text"
                                        value={memo}
                            ...
                            <div className="pt-8 border-t border-gray-100 dark:border-slate-800 flex justify-end">
                                ...
                                >
                                    {isLoading ? "Executing..." : "Authorize Withdrawal"}
                                </button>
                            </div>
                        </form>
                    )}
                ...
                {step === 1 && (
                    <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-800/30 rounded-3xl p-6 flex items-start gap-5 transition-colors">
                        ...
                        <div className="text-sm">
                            <h3 className="font-black uppercase tracking-wider text-red-900 dark:text-red-400 mb-1">Destructive Action Alert</h3>
                            <p className="text-red-800 dark:text-red-500/80 font-medium leading-relaxed">
                                Withdrawal operations immediately modify the customer ledger. Verification of liquidity and recipient identity is mandatory before execution.
                            </p>
                        </div>
                    </div>
                )}
