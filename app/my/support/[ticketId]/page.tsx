  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(`/api/support/${ticketId}/messages`, {
        method: "POST",
        body: JSON.stringify({ message: newMessage, sendAs: "Customer" }),
      });
      if (res.ok) {
        setNewMessage("");
    ...
    <div className="max-w-4xl mx-auto h-[calc(100vh-120px)] flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden animate-fade-in">
        ...
        {messages.map((msg, index) => {
          ...
          return (
            <div key={msg._id || index} className={`flex ${isCustomer ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                {!isCustomer && (
                  <p className="text-xs font-semibold text-gray-500 mb-1">Support Agent</p>
                )}
                <p className="whitespace-pre-wrap text-sm leading-relaxed">{msg.message}</p>
                <p className={`text-[10px] mt-2 text-right ${isCustomer ? 'text-blue-200' : 'text-gray-400'}`}>
      ...
      {ticket.currentStatus !== 'Closed' && ticket.currentStatus !== 'Resolved' ? (
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 text-center">
          <p className="text-sm text-gray-600">This ticket has been marked as {ticket.currentStatus}. If you need further assistance, please open a new ticket or reply to reopen.</p>
          <form onSubmit={handleSendMessage} className="flex gap-3 items-end justify-center mt-3 max-w-lg mx-auto">
             <div className="flex-1">
               <input
                value={newMessage}
                onChange={(e) => setNewMessage(e.target.value)}
