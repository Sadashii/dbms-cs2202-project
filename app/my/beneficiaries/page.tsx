            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                        Manage the people you pay often for quick internal
                        transfers.
                    </p>
                </div>
                <button
                ...
                <div className="py-20 text-center bg-white dark:bg-slate-900 border border-dashed border-gray-200 dark:border-slate-700 rounded-2xl shadow-sm">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto mb-6">
                        {search
                            ...
                            : "Save a VaultPay payee once and transfer to them instantly."}
                    </p>
                    {!search && (
                        <button
                            onClick={() => setIsAddOpen(true)}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map((b) => {
                        ...
                        return (
                            <div
                                key={b._id}
                                className="group bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-sm hover:shadow-md dark:hover:shadow-slate-800/60 transition-all p-5 flex flex-col gap-4"
                            >
                                <div className="flex items-start gap-3">
                                    <div
                                        className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold flex-shrink-0 ${avatarColor(b._id)}`}
                                    >
                                ...
                                <div className="space-y-1.5 text-xs text-gray-600 dark:text-gray-400">
                                    <div className="flex justify-between">
                                        <span className="text-gray-400 dark:text-gray-500 uppercase tracking-wide font-semibold text-[10px]">
                                            Account No.
                                        </span>
                                    ...
                                    {isDeleting_ ? (
                                        <div className="flex-1 flex items-center gap-1.5">
                                            <button
                                                ...
                                                disabled={isDeleting}
                                                className="flex-1 py-1.5 text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors disabled:opacity-50"
                                            >
                                                {isDeleting ? "..." : "Confirm"}
                                            </button>
                                            <button
                                                ...
                                                className="flex-1 py-1.5 text-xs font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
                                            >
                                                Cancel
                                            </button>
                                        </div>
            ...
            {beneficiaries.length > 0 && (
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center">
                    {filtered.length === beneficiaries.length
                        ? `${beneficiaries.length} saved recipient${beneficiaries.length === 1 ? "" : "s"}`
                        : `Showing ${filtered.length} of ${beneficiaries.length}`}
                </p>
            )}
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-800">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                    Save a person for quick transfers.
                                </p>
                            </div>
                            <button
                        ...
                        <form onSubmit={handleAdd} className="p-6 space-y-4">
                            {[
                                ...
                                <div key={id}>
                                    ...
                                    <input
                                        ...
                                        value={
                                            addForm[id as keyof typeof addForm]
                                        }
                                        onChange={(e) =>
                                            setAddForm((f) => ({
                                                ...f,
                            ...
                            <div className="flex gap-3 pt-2">
                                <button
                                    ...
                                    onClick={() => setIsAddOpen(false)}
                                    disabled={isSaving}
                                    className="flex-1 py-2.5 text-sm font-semibold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-xl transition-colors disabled:opacity-50"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    disabled={isSaving}
                                    className="flex-1 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                                >
                                    {isSaving ? (
                                        <>
