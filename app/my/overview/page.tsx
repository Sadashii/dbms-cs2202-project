...
const Skeleton = ({ className = "" }: { className?: string }) => (
    <div
        className={`bg-gray-200 dark:bg-slate-800/50 rounded-xl animate-pulse ${className}`}
    />
);
...
function StatCard({
    ...
    colorTheme: "blue" | "amber" | "red" | "purple" | "indigo";
    href: string;
}) {
    const colors = {
        blue: {
    ...
    return (
        <Link href={href} className="block h-full">
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm hover:shadow-md dark:hover:shadow-slate-800/50 transition-shadow p-5 flex gap-4 items-start h-full">
                <div
                    className={`flex-shrink-0 w-11 h-11 rounded-lg flex items-center justify-center ${colors[colorTheme].bg}`}
                >
                    <div className={colors[colorTheme].text}>{icon}</div>
                </div>
    ...
    return (
        <li className="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
            <div className="flex items-center gap-4 min-w-0">
                <div
                    className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold ${isCredit ? "bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" : "bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400"}`}
                >
                    {isCredit ? "↓" : "↑"}
                </div>
                <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                        {tx.memo ||
                            (tx.transactionId?.type === "TRANSFER"
                                ? "Wallet Transfer"
                                : "Transaction")}
                    </p>
            ...
            <div className="text-right flex-shrink-0 pl-4">
                <p
                    className={`text-sm font-bold ${isCredit ? "text-green-600 dark:text-green-400" : "text-gray-800 dark:text-gray-200"}`}
                >
                    {isCredit ? "+" : "−"}
                    {fmt(tx.amount)}
                </p>
            ...
            <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-16 px-4 sm:px-6 pt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    ...
                    <StatCard
                        ...
                        label="Next EMI Due"
                        value={
                            s?.nextEmiDue ? fmt(s.nextEmiDue.amount) : "No Due"
                        }
                        sub={
                            s?.nextEmiDue
                                ? new Date(
