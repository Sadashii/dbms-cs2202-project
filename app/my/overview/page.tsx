    ...
    <div className="rounded-2xl overflow-hidden border border-blue-100 dark:border-slate-800 bg-gradient-to-br from-blue-50 via-indigo-50 to-white dark:from-slate-900 dark:via-slate-800/50 dark:to-slate-900 shadow-sm transition-colors">
      <div className="p-8 md:p-10 flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="text-center md:text-left flex-1">
          <p className="text-xs font-semibold uppercase tracking-widest text-blue-500 dark:text-blue-400 mb-1">Welcome aboard</p>
        ...
        <div className="hidden lg:flex flex-col gap-4 w-60 flex-shrink-0">
          {[
            ...
            <div key={step} className="flex items-center gap-3">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${done ? "bg-green-500 text-white" : "bg-white dark:bg-slate-800 border-2 border-gray-300 dark:border-slate-600 text-gray-400 dark:text-gray-500"}`}>
                {done ? "✓" : step}
              </div>
              <span className={`text-sm font-medium ${done ? "text-green-700 dark:text-green-400" : "text-gray-500 dark:text-gray-400"}`}>{label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {!isNewUser && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            icon={<svg className="w-5 h-5 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" /></svg>}
          />
          <StatCard
            label="Next EMI Due"
            ...
            icon={<svg className="w-5 h-5 text-red-500 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>}
          />
          <StatCard
            label="Active Cards"
            ...
            icon={<svg className="w-5 h-5 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>}
          />
        </div>
      )}
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
            ...
            {(!data?.recentTransactions || data.recentTransactions.length === 0) ? (
              <div className="py-16 text-center text-gray-400 dark:text-gray-500">
                <svg className="mx-auto mb-3 w-10 h-10 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <ul className="divide-y divide-gray-50 dark:divide-slate-800/50">
                {data.recentTransactions.map((txn) => (
                  <li key={txn._id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold ${txn.entryType === "Credit" ? "bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" : "bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400"}`}>
                    ...
                    <div className="text-right flex-shrink-0 pl-4">
                      <p className={`text-sm font-bold ${txn.entryType === "Credit" ? "text-green-600 dark:text-green-400" : "text-gray-800 dark:text-gray-200"}`}>
                ...
                {data?.accounts.map((acc) => (
                  <li key={acc._id} className="px-5 py-4">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{acc.accountType}</span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${acc.currentStatus === "Active" ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" : "bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400"}`}>
                        {acc.currentStatus}
                      </span>
                    </div>
                    ...
                    <p className="text-xs font-mono text-gray-400 dark:text-gray-500 mt-0.5">****{acc.accountNumber.slice(-4)}</p>
                  </li>
                ))}
              </ul>
            </div>
